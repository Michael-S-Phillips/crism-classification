"""
Seamless mosaic comparison over the full MC13 Mars Chart quadrant (54 tiles):

  Panel 1 (top) — MAF band-parameter composite (R=OLINDEX3, G=LCPINDEX2, B=HCPINDEX2)
                  from each tile's mrrsu file. Pooled percentile stretch.
  Panel 2 (bottom) — v3 (denoising) classifier multi-label output with
                  two-stage thresholding:
                    (a) inverse-proportional thresholds derived from val_AP
                    (b) global minimum floor (default 0.8)
                  Pixels passing none → unclassified (white).
                  Pixels passing multiple classes → additive RGB blend.
                  plagioclase uses a SUBTRACTIVE (darkening) contribution.

Target raster is downsampled to ~5× native pixel size (~1 km/pixel) to keep
...
Effective aspect ratio ≈ 0.5–0.7 (taller than wide) → side-by-side panels.

Output: reports/v5/fig_v5_mc13_seamless_classifier_vs_params.png

Usage (no args needed):
    conda run -n crism python scripts/plot_mc13_classifier_vs_params.py
"""
...
# Probability channel order in saved npz
...
OUT_PATH = os.path.join(
    ...
    'fig_v5_mc13_seamless_classifier_vs_params.png',
)
...
def main():
    ...
    ax1.set_title(
        'v3 (denoising) classifier — multi-label\n'
    ...
    fig.suptitle(
        f'MC13 quadrant ({len(tiles)} tiles) — MAF band parameters vs v3 classifier output',
        fontsize=12,
    )
    ...
    print(f'  {"tile":<6}  {"valid":>10}  {"oli":>7}  {"lcp":>7}  {"hcp":>7}  '
          f'{"plag":>7}  {"unclass":>8}')
    grand = {k: 0 for k in ['olivine', 'lcp', 'hcp', 'plagioclase', 'unclassified']}
    for s in stats:
        nv = s['n_valid']
        for k in grand:
            grand[k] += s.get(k, 0)
