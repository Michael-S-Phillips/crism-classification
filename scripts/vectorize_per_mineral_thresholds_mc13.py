"""
Per-mineral, per-threshold vectorization for the v3 (denoising) classifier
output across all MC13 quadrant tiles.

Produces 4 GeoPackages — one per mineral — each containing 5 layers, one per
threshold value. Polygons in higher-threshold layers are nested subsets of
the same regions at lower thresholds (a pixel passing prob >= 0.97 also
passes >= 0.85).

Output layout:
  data/vector_mc13_v3_denoising/
    olivine.gpkg       (layers: thresh_0.85, _0.90, _0.93, _0.95, _0.97)
    lcp.gpkg           (layers: thresh_0.85, _0.90, _0.93, _0.95, _0.97)
    hcp.gpkg           (layers: thresh_0.85, _0.90, _0.93, _0.95, _0.97)
    plagioclase.gpkg   (layers: thresh_0.92, _0.94, _0.96, _0.97, _0.98)

Per-polygon columns:
  tile_id, mineral, threshold, count_px, mean_prob,
  band_00 … band_58 (mean reflectance per mrral band).

Light morphology applied (matches vectorize_multilabel_categories_mc13.py):
  - Per-class probability rasters median-filtered (size=3, 1 iteration)
...
Usage (no args needed):
    conda run -n crism python scripts/vectorize_per_mineral_thresholds_mc13.py
"""
...
DEFAULT_PROBS_DIR = '/tmp/v3_mc13'
DEFAULT_TILE_DIR  = '/Volumes/Mars_GIS/CRISM/MRDR/mc13'
DEFAULT_OUT_DIR   = os.path.join(PROJ, 'data', 'vector_mc13_v3_denoising')
DEFAULT_WAVELENGTHS_SIDECAR_NAME = 'mc13_v3_denoising_per_mineral_wavelengths.json'
