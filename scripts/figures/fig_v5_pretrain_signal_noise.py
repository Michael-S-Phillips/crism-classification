"""
Signal / noise decomposition comparison: denoising MAE vs SPEND MAE encoders.
...
For three representative pixels (olivine, hcp, plagioclase), shows:
  ...
  - Col 2: denoising MAE decomposition — two stacked subplots sharing x-axis:
           Top: signal estimate (recon, dashed) overlaid on clean (solid)
           Bottom: noise estimate (clean - recon), ±0.01 I/F y-range
  - Col 3: SPEND MAE decomposition — same stacked-subplot pattern
...
def main():
    ...
    n_cols = 3  # clean | denoising decomp | SPEND decomp
    ...
    for row_i, cls in enumerate(CLASSES_TO_SHOW):
        ...
        # ── Col 2: denoising MAE decomposition ──────────────────────────────
        plot_decomposition(
            fig, outer_gs[row_i, 1],
            wls, valid, center_clean, center_recon_denoising,
            color=color, model_label='denoising MAE',
            y_ref_lim=y_ref_lim,
        )

        # ── Col 3: SPEND MAE decomposition ──────────────────────────────────
        plot_decomposition(
            fig, outer_gs[row_i, 2],
            wls, valid, center_clean, center_recon_spend,
            color=color, model_label='SPEND MAE',
            y_ref_lim=y_ref_lim,
        )

        noise_denoising = center_clean - center_recon_denoising
        noise_spend = center_clean - center_recon_spend
        print(f'    noise MAE: denoising={np.abs(noise_denoising[valid]).mean():.5f}  '
              f'SPEND={np.abs(noise_spend[valid]).mean():.5f}  '
              f'center masked: denoising={bool(mask_denoising[CENTER_IDX])}  '
              f'SPEND={bool(mask_spend[CENTER_IDX])}')
