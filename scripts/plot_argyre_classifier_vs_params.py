"""
Seamless mosaic comparison over the Argyre 2-tile area (t0434, t0435):
...
Probabilities loaded from /tmp/v3_argyre/t*_probs.npz (produced by
classify_tile_supervised.py --save_probs).

Output: reports/v5/fig_v5_argyre_seamless_classifier_vs_params.png

Usage (no args):
    conda run -n crism python scripts/plot_argyre_classifier_vs_params.py
"""
...
# classify_tile_supervised.py CLASS_NAMES)
PROB_CHANNELS = ['olivine', 'lcp', 'hcp', 'plagioclase', 'other']

OUT_PATH = os.path.join(
    PROJECT_ROOT, 'reports', 'v5',
    'fig_v5_argyre_seamless_classifier_vs_params.png',
)
...
def main():
    ...
    # ── figure ────────────────────────────────────────────────────────────────
    # Two tiles side-by-side (wider than tall). Stack the two comparison panels
    # vertically so each panel gets enough vertical space.
    xmin, ymin, xmax, ymax = bounds
    lat_mid = (ymin + ymax) / 2
    cos_mid = math.cos(math.radians(lat_mid))
    # Aspect of the mosaic footprint (latitude range / (longitude range * cos(lat)))
    aspect = (ymax - ymin) / ((xmax - xmin) * cos_mid)

    # Layout: 2 rows (band params, classifier) × 1 column each.
    # Panel width chosen so the mosaic fills nicely.
    panel_w = 14
    panel_h = panel_w * aspect
    fig_h = panel_h * 2 + 2.5  # two panels + legend/title space

    fig, axes = plt.subplots(
        2, 1, figsize=(panel_w, fig_h),
    )
    ...
    fig.suptitle(
        'Argyre 2-tile mosaic (MC26) — MAF band parameters vs v3 classifier output',
        fontsize=12,
    )
