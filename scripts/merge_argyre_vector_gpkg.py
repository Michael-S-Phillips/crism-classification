"""
Merge the 2 per-tile, multi-layer Argyre vector GPKGs into a single
...
Source: data/vector_argyre_v3_denoising/*_mineral_map.gpkg
Output: data/vector_argyre_v3_denoising/argyre_v3_denoising_minerals.gpkg

Per-polygon columns:
  tile_id      str   — e.g. 't0434'
...
Usage (no args):
    conda run -n crism python scripts/merge_argyre_vector_gpkg.py
"""
...
OUT_PATH = os.path.join(VECTOR_DIR, 'argyre_v3_denoising_minerals.gpkg')
...
def tile_id_from_basename(path: str) -> str:
    """`/path/to/t0434_mrral_40s318_0327_4_mineral_map.gpkg` → `t0434`."""
    base = os.path.basename(path)
    # Strip suffix to get 't0434_mrral_40s318_0327_4'
...
def main():
    ...
    paths = [p for p in paths if not p.endswith('argyre_v3_denoising_minerals.gpkg')]
    if not paths:
